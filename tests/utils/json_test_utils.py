...
class DiffsReporter(object):
    ...
    def report_diffs(diffs, filename):
        ...
        with open(filepath, "w") as f:
            f.write(
                "{}/{} <- paths/total diffs\n{}".format(
                    stat_info["count"], len(diffs), stat_info["detail"]
                )
            )
            for diff in diffs:
...
def __validate_two_flat_lists(address, f_list, other_list):
    ...
    res.extend(
        assert_equal(
            ...
            f_len,
            other_len,
            "first list has {0} items while second list has {1} items".format(
                f_len, other_len
            ),
        )
    )
    ...
    for item in other_list:
        res.extend(
            assert_in(
                address, item, other_list, "{0} does not exists in {1}".format(item, s)
            )
        )
    return res


def __validate_list_keys(
    address, dict_jsons, other_dict_jsons, identity_fields, object_name=None
):
    res = []
...
def __validate_two_list_jsons(
    address, list_jsons, other_list_jsons, identity_fields, object_name, join_only=False
):
    """
    ...
    other_dict_jsons = __build_dict_from_list_json(
        other_list_jsons, identity_fields, object_name
    )

    res = __validate_list_keys(
        address, dict_jsons, other_dict_jsons, identity_fields, object_name
    )
        ...
        if join_only:
            res.extend(
                validate_two_nested_jsons_joining(
                    address, dict_jsons[key], other_dict_jsons[key]
                )
            )
        else:
            res.extend(
...
def validate_two_nested_jsons_joining(
    address, json_obj, other_json_obj, ignored_list=None
):
    """
        ...
        if diff:
            ...
            if type(json_obj[field]) is list:
                if field in MAPPING.keys():
                    ...
                    res.extend(
                        __validate_two_flat_lists(
                            new_address, json_obj[field], other_json_obj[field]
                        )
                    )
                ...
                res.extend(
                    assert_equal(
                        ...
                        json_obj[field],
                        other_json_obj[field],
                        "field {0}: {1} is not equal {2}".format(
                            field, json_obj[field], other_json_obj[field]
                        ),
                    )
                )
    return res
