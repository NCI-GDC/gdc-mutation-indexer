...
class TestJsonObject:
    ...
    @classmethod
    def validate_two_flat_lists(cls, f_list, other_list):
        f_len = len(f_list)
        other_len = len(other_list)
        assert len(f_list) == len(other_list),\
            "first list has {0} items while second list has {1} items".format(f_len, other_len)
        s = set(f_list)
        for item in other_list:
            assert item in other_list, "{0} does not exists in {1}".format(item, s)

    @classmethod
    def validate_two_flat_jsons(cls, json_obj, other_json_obj):
        for field in json_obj.keys():
            assert field in other_json_obj.keys(), "{0} does not exists in {1}".format(field, other_json_obj.keys())
    ...
    def validate_two_list_jsons(cls, list_jsons, other_list_jsons, identity_fields, diff_func=None, object_name=None):
        ...
        assert first_size == second_size, \
            "First list has {0} items while second list has {1} items".format(first_size, second_size)

        for key in dict_jsons.keys():
            assert key in other_dict_jsons.keys(), "{0} is not in {1}".format(key, other_list_jsons.keys())
            if diff_func:
    ...
    def validate_two_nested_jsons(cls, json_obj, other_json_obj, ignore_list=None):
        if ignore_list is None:
            ignore_list = []
        for field in json_obj.keys():
            assert field in other_json_obj.keys(), "{0} is not in {1}".format(field, other_json_obj.keys())
