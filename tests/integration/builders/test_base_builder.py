...
@pytest.fixture
def mapping_data(input_dir: str) -> dict:
    with open(path.join(input_dir, "case_centric_mapping.json")) as mapping_fp:
        return json.load(mapping_fp)


def test__get_all_boolean_paths__all_paths_returned(mapping_data):
...
def test_sample_data_cast_boolean(
    mapping_data: dict, spark_session: sql.SparkSession, input_dir: str
) -> None:
    df = spark_session.read.parquet(path.join(input_dir, "sample.parquet"))
...
def assert_all_paths_are_booleans(
    schema: types.StructType, paths: Iterable[Iterable[str]]
) -> None:
    for path in paths:
