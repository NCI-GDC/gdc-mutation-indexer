...
import config
from exports import builders, configuration
...
def test_cnv_subtree(
    ...
    cons_df = builders.ConsequenceBuilder(default_old_config, sqlContext).build_for_cnv(
    ...
    true_stats = join_utils.get_relationship_map(
        df, ("cnv_id", "consequence_id", "gene_id")
    )
    ...
    stats = join_utils.get_relationship_map(df, ("cnv_id", "consequence_id", "gene_id"))
...
def test_cnv_occurrence_centric_count(
    ...
    built_count = es_client.count(
        index=default_config.elasticsearch.write.indices[
