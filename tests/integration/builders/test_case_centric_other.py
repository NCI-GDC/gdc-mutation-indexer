...
@pytest.mark.usefixtures(
    "maf_df", "gistic_df", "all_cases", "all_maf_cases", "case_centric_df", "test_data"
)
class TestCaseCentricOther:
    """Other case centric tests"""

    def test_empty_cases(
        self, case_centric_df, maf_df, gistic_df, test_data, all_cases, all_maf_cases
    ):
        """
        Test "empty cases"
        ...
        stats = true_stats.TestDataStats.get_stats(
            maf_df, gistic_df, test_data, "case_centric"
        )
    ...
    def test_available_variation_data(
        self, case_centric_df, maf_df, gistic_df, all_cases, all_maf_cases, test_data
    ):
        """
    ...
    @pytest.mark.parametrize("path", mapper.ModelMapper("case_centric").get_paths())
