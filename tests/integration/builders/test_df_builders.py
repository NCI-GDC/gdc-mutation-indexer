...
class TestDFBuildersBase:
    ...
    def params():
        return itertools.chain(
            itertools.product(
                conf.main_indices, ["transcript", "cnv", "ssm", "gene", "annotation"]
            ),
            itertools.product(
                conf.ssm_indices, ["transcript", "ssm", "gene", "annotation"]
            ),
            itertools.product(conf.cnv_indices, ["cnv", "gene"]),
        )
...
class TestDFBuilders(TestDFBuildersBase):
    ...
    def test_simple_df_build(self, index_type, df_type, get_inputs):
        ...
        must_have_keys = [
            k for k in mapping.keys() if all([-1 == k.find(s) for s in stopwords])
        ]
    ...
    def test_df_drop_fields(self, index_type, df_type, get_inputs):
        ...
        df = get_function(input_df, index_type, drop_fields=fields_to_delete)
    ...
    def test_unique_fields(self, index_type, df_type, get_inputs):
        ...
        df_unique = get_function(
            input_df, index_type, unique_fields=self.unique_fields[df_type]
        )
        assert df_unique.count() == (
            df.select(self.unique_fields[df_type]).distinct().count()
        )
