...
from tests.integration import config as test_config
...
class TestBaseConfig(object):
    ...
    def base_config(self, setup_graph_indices, monkeypatch):
        ...
        """
        monkeypatch.setattr(client.IndexClient, "get", self._stub_indexd_get_maf)
        monkeypatch.setattr(config.BaseConfig, "list_bucket", self._stub_list_bucket)

        env_dict = test_config.TestConfig.get_env_dict()
        base_config = config.BaseConfig(env_dict=env_dict)
    ...
    def _stub_indexd_get_maf(cls, did):
        ...
        """
        matching_urls = [url for url in conf.maf_urls if did in url]
        ...
        return client.Document(client=None, did=did, json=doc_json)
    ...
    def _stub_list_bucket(cls, bucket_name):
        ...
        """
        assert bucket_name == conf.s3_gistic_bucket
        return [FakeS3Key(url) for url in conf.gistic_urls]
    ...
    def _reformat_gistic_url(cls, url):
        ...
        """
        return "{bucket}{url}".format(bucket=conf.s3_gistic_bucket, url=url)
    ...
    def test_get_maf_urls(self, base_config):
        ...
        expected_urls = sorted(self._reformat_maf_url(url) for url in conf.maf_urls)
    ...
    def test_get_gistic_urls(self, base_config):
        ...
        expected_urls = sorted(
            self._reformat_gistic_url(url) for url in conf.gistic_urls
        )
