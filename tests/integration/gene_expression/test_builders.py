...
def expression_value_df(
    ...
    return builders.ExpressionValueBuilder(
...
def ge_builder(
    ...
    spark_session: sql.SparkSession,
    es_client: elasticsearch.Elasticsearch,
) -> Iterable[builders.IndexBuilder]:
...
def ge_file_docs(
    ...
    with test_setup.IndexManager(
        ...
        with test_setup.DocumentLoader(ge_config, es_client) as loader:
...
def test_gene_expression_builder(
    ge_config: configuration.Configuration,
    ge_builder: builders.IndexBuilder,
    es_client: elasticsearch.Elasticsearch,
