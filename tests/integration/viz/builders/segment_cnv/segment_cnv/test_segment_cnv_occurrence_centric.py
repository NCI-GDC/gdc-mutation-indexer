...
def segment_cnv_occurrence_centric_builder(
...
) -> Iterator[builders.SegmentCNVOccurrenceCentricBuilder]:
    ...
    with test_setup.IndexManager(
        segment_config,
        es_client,
        index_types=(build.IndexType.CNV_OCCURRENCE_CENTRIC,),
        skip_creation=True,
    ):
...
def test__segment_cnv_occurrence_centric_builder(
    segment_config: configuration.Configuration,
    segment_cnv_occurrence_centric_builder: builders.SegmentCNVOccurrenceCentricBuilder,
    es_client: elasticsearch.Elasticsearch,
