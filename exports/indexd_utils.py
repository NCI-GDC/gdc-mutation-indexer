...
class IndexdDocumentDataFrameUtil:
    ...
    def _get_and_format_url(self, doc: client.Document) -> Optional[str]:
        """
    ...
    def _get_dataframe(
        self, urls: Union[str, List[str]], schema: Optional[types.StructType], include_file_name: bool
    ) -> sql.DataFrame:
        enforce_schema = bool(schema) or None
    ...
    def get_dataframe(
        ...
        urls = iter(doc_url.url for doc_url in doc_urls)
        batches = more_itertools.ichunked(urls, batch_size)
        
        document_df = self._get_dataframe(list(next(batches)), schema, include_document_ids)

        for batch in batches:
            batch_df = self._get_dataframe(list(batch), schema, include_document_ids)

            document_df = document_df.union(batch_df)

        if include_document_ids:
            url_df = self._sql_context.createDataFrame(doc_urls, ("did", "_input_file_name"))
            
            return document_df.join(url_df, on=["_input_file_name"]).drop("_input_file_name")
