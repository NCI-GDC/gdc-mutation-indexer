...
class GeneForJoinsCentricBuilder(BaseBuilder):
    """
    Builds gene-centric dataframe given gistic and maf dataframes:

        gene{}
             |___ ...
    """

    index_name = 'gene_for_joins_centric'
    id_field = 'gene_id'

    def build(self, maf_df, gistic_df, case_df):
        """
        Builds Gene For Joins Centric index
        """
        self.log('Building GeneForJoinsCentric')
        # Check if we should load a pre-built dataframe
        if self.config.index_use_existing:
            self.gene_for_joins_centric = self.get_existing()
            if self.gene_for_joins_centric is not None:
                return self
        ...
        self.gene_for_joins_centric = gene_df
