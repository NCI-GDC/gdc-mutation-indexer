...
class PrimaryAliquotBuilder(base_input_builder.BaseInputBuilder):
    ...
    def __init__(
        ...
        es_datafram_util: es_utils.ElasticsearchDataFrameUtil,
    ):
        ...
        self._config = config
        self._logger = logger
        self._es_dataframe_util = es_datafram_util
    ...
    def _get_primary_aliquots(
        ...
        files_df = self._es_dataframe_util.get_dataframe(
            es_utils.Index.File,
    ...
    def _get_main_urls(self, file_ids: Iterable[str]) -> Iterable[Mapping[str, str]]:
        ...
        docs = itertools.chain.from_iterable(
            self._config.indexd.bulk_request(list(bids)) for bids in batches
        )
    ...
    def build_from_scratch(self) -> sql.DataFrame:
        ...
        include_fields = {"cases.samples.portions.analytes.aliquots.aliquot_id"}

        return (
            self._get_primary_aliquots(filters, include_fields)
            .select(
                "case_id",
                "file_id",
                "experimental_strategy",
