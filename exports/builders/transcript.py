import logging
from pyspark.sql.functions import explode, col, collect_list, struct, udf
...
class TranscriptBuilder(object):
    ...
    def build(self, maf_df, join_gene=False):
        ...
        '''
        ann_df = struct(get_annotation_df(maf_df, unique_fields=['transcript_id'])).alias('annotation')
        ...
        tran_with_ann = tran_df.join(ann_df, tran_df.transcript_id == ann_df.annotation.transcript_id, how='left')\
            .select('annotation', *tran_df.columns)
