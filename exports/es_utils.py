...
class ElasticsearchDataFrameUtil:
    def __init__(self, sql_context: sql.SQLContext, config):
        self._sql_context = sql_context
        self._config = config

    def _index_to_str(self, index: Index) -> str:
        if index == Index.File:
            return "{}".format(self._config.graph_file_index)

        if index == Index.Case:
            return "{}".format(self._config.graph_case_index)

        raise Exception("Index not conigured: {}.".format(index))
