...
class DocumentResolver:
    ...
    def __init__(
        self, config: configuration.Read, es_client: elasticsearch.Elasticsearch
    ) -> None:
        self._config = config
    ...
    def get_ids(self, projects: Sequence[str]) -> Sequence[str]:
        ...
        hits = es_utils.iterate_es_results(
            self._es_client,
            index_name=self._config.indices[constants.IndexType.FILE],
            query=body,
        )
...
class ASCATBuilder(bases.InputBuilder[configuration.ASCATBuilder, ASCATInputs]):
    ...
    def _build_file_df(self, file_ids: Iterable[str]) -> sql.DataFrame:
        ...
        return (
            self._es_dataframe_util.read(constants.IndexType.FILE, query=body)
            .select(
                "file_id",
