...
class GisticBuilder(bases.BaseInputBuilder):
    """
    ...
    def _add_cnv_id(self, gistic_df):
        ...
        """
        gistic_df = gistic_df.withColumn(
            "cnv_id",
            utils.uuid5_col(
                F.col("chromosome"),
    ...
    def _add_occurrence_id(self, gistic_df):
        ...
        """
        gistic_df = gistic_df.withColumn(
            "occurrence_id", utils.uuid5_col(F.col("cnv_id"), F.col("case_id"))
        )
        return gistic_df
    ...
    def _add_observation_id(self, gistic_df):
        ...
        gistic_df = gistic_df.withColumn(
            "observation_id",
            utils.uuid5_col(F.col("cnv_id"), F.col("case_id"), F.col("aliquot_id")),
        )
        return gistic_df
    ...
    def _trim_gene_symbol(self, initial_cnv_df):
        ...
        trim_gene_symbol_udf = F.udf(trim_gene_symbol_inner, types.StringType())
        trimmed_df = initial_cnv_df.withColumn(
            "gene_id", trim_gene_symbol_udf(F.col("Gene Symbol"))
        )
    ...
    def _add_gene_information(
        self, gistic_df: sql.DataFrame, gene_model_df: sql.DataFrame
    ) -> sql.DataFrame:
        gene_to_cnv_col_names = {
        ...
        new_df = gistic_df.join(
            gene_model_df, gistic_df.gene_id == gene_model_df._gene_id
        )

        # Create cnv columns from gene model
        for old, new in gene_to_cnv_col_names.items():
            new_df = new_df.withColumn(new, F.col(old))
    ...
    def _add_ncbi_build(self, initial_cnv_df):
        ...
        cnv_df_with_ncbi_build = initial_cnv_df.withColumn(
            "ncbi_build", F.lit("GRCh38")
        )
    ...
    def _add_gene_level_cn(self, initial_cnv_df):
        ...
        cnv_df_with_gene_level_cn = initial_cnv_df.withColumn(
            "gene_level_cn", F.lit(True)
        )
    ...
    def _add_variant_fields(self, initial_df):
        ...
        """
        new_df = initial_df.withColumn(
            "variant_status", F.lit("Tumor only")
        ).withColumn("variant_caller", F.lit("GISTIC2"))
        return new_df
    ...
    def _add_available_variation_data(self, gistic_df):
        ...
        gistic_df = gistic_df.withColumn("available_variation_data", F.lit("cnv"))
    ...
    def _add_case_id(self, df):
        ...
        aliquot_ids = (
            df.select("aliquot_id").distinct().rdd.map(lambda x: x[0]).collect()
        )

        # query all case_documents that have relevant aliquots attached
        query = {
            "query": {
                "constant_score": {"filter": {"terms": {"aliquot_ids": aliquot_ids}}}
            },
            "_source": ["aliquot_ids"],
        }

        relevant_cases = es_utils.iterate_es_results(
        ...
        df = utils.map_create_column(df, map_aliquot_to_case, "aliquot_id", "case_id")
    ...
    def _cnv_change_to_string_and_drop_zero(self, df):
        ...
        cnv_change_mapping = {"-1": "Loss", "0": None, "1": "Gain"}
        ...
        stringify_cnv_change_udf = F.udf(stringify_cnv_change_inner, types.StringType())
        new_df = df.withColumn(
            "cnv_change", stringify_cnv_change_udf(F.col("cnv_change_init"))
        )
