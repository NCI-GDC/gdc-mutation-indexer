...
def get_case_ids_from_source_es(
    ...
    for aliquot_node in aliquot_df.toLocalIterator():
        if aliquot_node.project_id:
            # If we were configured only to build certain projects, then there's no
            # point in tracking aliquots from other projects.
            if (not project_filter) or aliquot_node.project_id in project_filter:
                submitter_ids_by_project[aliquot_node.project_id].add(
                    aliquot_node.submitter_id
                )
        else:
            floating_submitter_ids.add(aliquot_node.submitter_id)
