...
async def run_spark_command(config: configuration.Configuration) -> None:
    ...
    with open(output_file, "wb+") as out_f, open(
        error_file, "wb+"
    ) as error_f, resources.path(
        "mutation_indexer.master", "driver_wrapper.py"
    ) as driver:
        final_command = " ".join(
            more_itertools.value_chain(
                spark_command, arguments, driver, master.Driver.VIZ.name
            )
        )
